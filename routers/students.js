...
router.get('/:id',(req,res)=>{
    ...
    if(!student)
        return res.status(404).send('Student with id is not found');
    res.send(student);
});
...
router.post('',(req,res) =>{
    ...
    //    return res.status(400).send('Name is required with 3 charcters at least')
    let vald_res = Joi.validate(req.body, validation_schema);
    if(vald_res.error)
...
router.put('/:id',(req,res)=>{
    ...
    vald_res = Joi.validate(req.body, validation_schema);
    if(vald_res.error)
        return res.status(400).send(vald_res.error.details[0].message)
